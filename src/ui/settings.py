...
from src.ui.nightlord_selector import NightlordSelectorDialog
...
""".strip())
    ...
    def open_nightlord_selector(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if selection:
                self.updater.manual_detect_and_update_map((selection.nightlord,))  # 触发一次识别
                self.update_manual_constraint_label(selection.nightlord)
                info(f"Manual map constraint set: Nightlord={selection.nightlord}")
            else:
                warning("Nightlord selection returned without valid data")
        else:
            info("Nightlord selection canceled")

    def update_manual_constraint_label(self, nightlord: int | None):
        """更新手动限定范围的标签显示"""
        if nightlord is None:
            self.manual_constraint_label.setText("手动限定范围：未设置")
        else:
            # 使用和NightlordSelectorDialog相同的名称映射
            nightlord_names = {
                0: "三头野兽", 1: "碎身巨鳄", 2: "慧心虫", 3: "征兆",
                4: "平衡律法的魔物", 5: "暗中飞驰的猎人", 6: "雾中裂缝", 7: "黑夜化形者",
            }
            nightlord_name = nightlord_names.get(nightlord, f"未知({nightlord})")
            self.manual_constraint_label.setText(f"✔️手动限定范围：夜王={nightlord_name}")
